...
const loginAdmin = async (req, res) => {
    ...
    try {
        const [rows] = await connection.query('SELECT * FROM users WHERE phone = ? AND password = ? AND level=1 ', [username, md5(pwd)]);
        if (rows.length == 1) {
            if (rows[0].status == 1) {
                const { password, money, ip, veri, ip_address, status, time, ...others } = rows[0];
                const accessToken = jwt.sign({
...
const sendOtp = async (req, res) => {
  ...
  const data=await axios.post(`https://www.full2pay.in/api_sms.php?token=IZP7nDi9GM4hV4e31dHk&mobile=${phone}&otp=${otp}&expiry=10&brand=macid`)

 
            
        if (data.data.status==="success") {
            await connection.query("UPDATE users SET otp = ?, time_otp = ? WHERE phone = ? ", [otp, timeEnd, phone]);
            return res.status(200).json({
...
// https://www.full2pay.in/api_sms.php?token=IZP7nDi9GM4hV4e31dHk&mobile=
// 9999999990&otp=123456
// &expiry=10&brand=Full2Pay.in
...
const verifyCodeforregister = async (req, res) => {
    ...
    let timeEnd = (+new Date) + 1000 * (60 * 2 + 0) + 500;
    let otp = randomNumber(100000, 999999);
    request(`https://www.fast2sms.com/dev/bulkV2?authorization=yj8Xb5LDd3W1z6pvYA7PCFKusahoJTNIrqfE90ROMGtmkVeQx4CNp9FHujnxZVWwBg2vL6KeUMyksqXI&route=otp&variables_values=${otp}&flash=0&numbers=${phone}`, async (error, response, body) => {
            ...
            if (check_i.length == 1) {
                if (check_ip.length <= 3) {
                    ...
                    if (check_i[0].level == 2) {
                       ...
                       const [rows] = await connection.execute('SELECT COALESCE(MAX(id_user), 0) AS max_id FROM users');
...
const verifyCode = async (req, res) => {
    ...
    if (rows.length == 0) {
        await request(`https://www.fast2sms.com/dev/bulkV2?authorization=Cl8g0hU2NaGfsKdRexa2kH5Z3ObWMIN7fObXsLRW7IJFmDST9bbcU6cO7wMQ&variables_values=${otp}&route=otp&numbers=${phone}`, async (error, response, body) => {
            let data = JSON.parse(body);
            if (data.code == '00000') {
                await connection.execute("INSERT INTO users SET userPhoto=?, phone = ?, otp = ?, veri = 0, time_otp = ? ", [lastDigit,phone, otp, timeEnd]);
                return res.status(200).json({
        ...
        if (user.time_otp - now <= 0) {
            request(`https://www.fast2sms.com/dev/bulkV2?authorization=Cl8g0hU2NaGfsKdRexa2kH5Z3ObWMIN7fObXsLRW7IJFmDST9bbcU6cO7wMQ&variables_values=${otp}&route=otp&numbers=${phone}`, async (error, response, body) => {
                let data = JSON.parse(body);
                if (data.code == '00000') {
                    await connection.execute("UPDATE users SET otp = ?, time_otp = ? WHERE phone = ? ", [otp, timeEnd, phone]);
                    return res.status(200).json({
...
try{
    ...
    if (rows.length == 0) {
        ...
        if (user.time_otp - now > 0) {
            if (user.otp == otp) {
                await connection.query("UPDATE users SET password = ?, otp = ?, time_otp = ? WHERE phone = ? ", [md5(pwd), otp, timeEnd, username]);
                return res.status(200).json({
...
const sendOTPOnEmail=async(req,res)=> {
    ...
    const [users] = await connection.query('SELECT * FROM users WHERE `phone` = ? ', [userInfo.phone]);
...
const submitEmail=async(req,res)=> {
  const {otp,email}=req.body
    ...
    try {
        ...
        const [user] = await connection.query('SELECT `phone`, `otp`,`email` FROM users WHERE `token` = ? ', [auth]);
    let userInfo = user[0];
...
const loginEmail=async(req,res)=> {
    ...
    if (!email || !pwd) {//!isNumber(username)
        return res.status(200).json({
            message: 'Please enter all field',
            status:false
        });
    ...
    try {
        const [rows] = await connection.query('SELECT * FROM users WHERE email = ? AND password = ? ', [email, md5(pwd)]);
        if (rows.length == 1) {
            if (rows[0].status == 1) {
                const { password, money, ip, veri, ip_address, status, time, ...others } = rows[0];
                const accessToken = jwt.sign({
                ...
                const sql_login = `INSERT INTO login SET phone = ?`;
                await connection.execute(sql_login, [rows[0].phone]);
                await connection.execute('UPDATE `users` SET `token` = ?,`login_at`=CURRENT_TIMESTAMP WHERE `email` = ? ', [md5(accessToken), email]);
