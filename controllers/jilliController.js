...
const agentId = "moremoney_INR";
const agentKey = "9db149271f5e94dece96214f7e42b3305e900dc2";
const lang = 'en-US';
const api = `https://wb-api-2.richjourney888.com/api1`
...
async function login(account, gameId, retryCount = 0) {
    ...
    const apiUrl = `${api}/LoginWithoutRedirect`;
    try {
        ...
        if (response.response.statusCode === 200) {
            ...
            if (responseBody && responseBody.ErrorCode === 14 && responseBody.Message === "User not exist or not enabled!") {
                ...
                return decodeURIComponent(datas.Data);
            }
...
const boardGame = async (req, res) => {
    ...
    const { id, id_user, phone, password, money, ip, veri, ip_address, status, time, token, ...others } = rows[0];
    ...
    if (response &&  response.length) {
        ...
        await connection.query('UPDATE users SET money = money - ? WHERE phone = ? ', [money, phone]);
        
        let { Data } = await exchangeTransferByAgentId(username, money, 2, transactionId);
      
        if (Data ) {
            console.log("oijijio")
            await connection.execute(sql, [transactionId, phone, money, 'jilli', Data.Status, checkTime, dates]);
...
const transferMoneyToMainWallet = async (req, res) => {
    try {
        ...
        if (response.Data && response.Data.length) {
            ...
            // Perform transfer
            let transferResponse = await exchangeTransferByAgentId(username, data.Balance, 1, transactionId);
            ...
            if (Data && Object.keys(Data).length) {
                ...
                // Update user balance
                await connection.query('UPDATE users SET money = money + ? WHERE phone = ?', [data.Balance, phone]);
