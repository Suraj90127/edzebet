        ...
        <>
            <div className='nav-bg p-1 py-3 sticky top-0 z-10'>
                <div className="container-section flex  items-center relative">
                    ...
                    <h1 className='heading-h1 gray-50 text-center flex justify-center items-center m-auto'>Change avatar</h1>
                </div>
            </div>
            ...
            <div className="container-section p-2 mt-2">
                <div className="grid grid-cols-12 gap-3">
                    ...
                    {avatarArray.map((item, index) => (
                        <div className={`col-span-4 rounded-lg relative ${userInfo?.userPhoto==index?"border-2 border-[--bg-color-l]":""}`} key={index}  >
                            <img src={item} alt={`Avatar ${index + 1}`} className={`rounded-lg ${userInfo?.userPhoto==index?"border-2 border-[--bg-color-l]":""}`}
                            onClick={()=>handleUpdate(index)} loading="lazy" 
                            />
