...
const AddBankCard = () => {
  const { successMessage, addBankData, userInfo } = useSelector(
    (state) => state.auth
  );
  ...
  const [state, setState] = useState({
    ...
    stk: "", ////account number
    email: "0", // ifsc code
    tinh: userInfo.phone_user, //phone number
    sdt: "", //usdt
  });
    ...
    <>
      ...
      <div className="container-section">
        ...
        {/* <div className="mt-7">
          ...
          <input
            type="text"
            className="w-full mt-2 nav-bg border border-slate-900 rounded-md p-2 focus:border-slate-700 ps-6 flex items-center focus:border focus:outline-none  placeholder:text-sm placeholder:text-slate-500"
        ...
        <div className="mt-7">
          ...
          <input
            type="text"
            className="w-full mt-2 nav-bg border border-slate-900 rounded-md p-2 focus:border-slate-700 ps-6 flex items-center focus:border focus:outline-none  placeholder:text-sm placeholder:text-slate-500"
            placeholder="Please enter the recipient's usdt address"
            name="sdt"
            onChange={inputHandle}
            value={state.sdt}
          />
        ...
        <div className="mt-7">
          <div className="flex ">
            ...
            <p className="text-sm gray-50">TRX Address</p>
          </div>
          <input
            type="text"
            className="w-full mt-2 nav-bg border border-slate-900 rounded-md p-2 focus:border-slate-700 ps-6 flex items-center focus:border focus:outline-none  placeholder:text-sm placeholder:text-slate-500"
            placeholder="Please enter the recipient's trx address"
            name="stk"
            onChange={inputHandle}
            value={state.stk}
          />
