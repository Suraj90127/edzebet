...
export const resetPasword = createAsyncThunk(
  "auth/reset-pass",
  async (
    { username, password, password2, otp },
    { rejectWithValue, fulfillWithValue }
  ) => {
    try {
      const { data } = await api.post("/resetPasword", {
        username: username,
        pwd: password,
        cpass: password2,
        otp: otp,
...
export const recharge3 = createAsyncThunk(
  ...
  async ({ amount, type }, { rejectWithValue, fulfillWithValue }) => {
    try {
      const { data } = await api.post('/webapi/rechargepay', { amount: amount, type: type }, { withCredentials: true });
      return fulfillWithValue(data);
    } catch (error) {
      return rejectWithValue(error.response.data);
    }
...
export const randompromotion = createAsyncThunk(
  ...
  async (_, { rejectWithValue,fulfillWithValue }) => {
    try {
      const { data } = await api.get(`/webapi/random-promotion`,{withCredentials:true});
        return fulfillWithValue(data); // Return data directly
    } catch (error) {
      return rejectWithValue(
        error.response ? error.response.data : 'An unknown error occurred'
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(rechargeList2.fulfilled, (state, { payload }) => {
        const rechargelistData2 = payload.datas;
        state.successMessage = payload.message;
        state.loader = false;
        state.rechargelistData2 = rechargelistData2;
      })
