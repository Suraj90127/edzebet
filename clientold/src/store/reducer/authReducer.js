...
export const login = createAsyncThunk(
  ...
  async ({username, pwd}, { rejectWithValue, fulfillWithValue }) => {
    try {
      const { data } = await api.post('/webapi/login', {username:username, pwd:pwd});
...
export const sendOtp = createAsyncThunk(
  ...
  async (username, { rejectWithValue, fulfillWithValue }) => {
    try {
      const { data } = await api.post('/sent/sendOtp', {username:username});
      // localStorage.setItem('token', data.user.token);
      return fulfillWithValue(data);
    } catch (error) {
      return rejectWithValue(error.response.data);
...
export const resetPasword = createAsyncThunk(
  'auth/reset-pass',
  async ({username,password,password2,otp}, { rejectWithValue, fulfillWithValue }) => {
    try {
      const { data } = await api.post('/resetPasword', {username:username,pwd:password,cpass:password2,otp:otp});
      // localStorage.setItem('token', data.user.token);
      return fulfillWithValue(data);
    } catch (error) {
      return rejectWithValue(error.response.data);
...
export const recharge2 = createAsyncThunk(
  ...
  async ({amount,type}, { rejectWithValue, fulfillWithValue }) => {
    try {
       const { data } = await api.post('/webapi/payments', {amount:amount,type:type}, { withCredentials: true });
      return fulfillWithValue(data);
    } catch (error) {
      return rejectWithValue(error.response.data);
    }
...
export const withdrawal = createAsyncThunk(
  ...
  async ({type,money,password}, { rejectWithValue, fulfillWithValue }) => {
    try {
      const { data } = await api.post('/webapi/withdrawal',{type,money,password}, {withCredentials:true});
      return fulfillWithValue(data);
    } catch (error) {
      return rejectWithValue(error.response.data);
    }  
...
export const userProblem = createAsyncThunk(
  ...
  async (formData, { rejectWithValue,fulfillWithValue }) => {
    try {
      const { data } = await api.post(`/webapi/userProblem`,formData,{withCredentials:true});
...
export const userProblemget = createAsyncThunk(
  ...
  async (progressForm, { rejectWithValue,fulfillWithValue }) => {
    try {
      const { data } = await api.post(`/webapi/userProblem-get`,progressForm,{withCredentials:true});
...
notificationgetData:null
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(resetPasword.rejected, (state, { payload }) => {
        // console.log('register rejected payload:', payload); // Log payload
        state.errorMessage = payload?.errorMessage || 'An error occurred';
        state.loader = false;
      })
