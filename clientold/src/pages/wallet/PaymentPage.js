  ...
  const handleSubmit = async () => {
    ...
    useEffect(() => {
      dispatch(rechargeGet()).then((res) => {
        ...
        const upiLink = `upi://pay?pa=${encodeURIComponent(res.payload.infoBank[0].stk)}&pn=${encodeURIComponent('Merchant')}&am=${encodeURIComponent(rechargegetData?.money)}&cu=INR`;
        setPay1(upiLink);
        ...
        <div className=" ">
          <div className='nav-bg p-1 py-3 sticky top-0'>
            <div className="container-section flex  items-center relative">
              <button className='absolute'><Link to={"/wallet/Recharge"}>  <IoIosArrowBack className='text-xl' /></Link></button>
          ...
          <div className='bgs-green mt-3 p-4'>
            ...
            <div className=" text-white py-4">
              <p className="text-3xl font-bold">{rechargegetData?.type == "UPI" ? `₹${(rechargegetData?.money)?.toFixed(2)}` : `$${((rechargegetData?.money) / 92)?.toFixed(2)}`} </p>
            </div>

          </div>
          <div className=" my-4"></div>
          <div className="text-center mb-4">
            <p className="font-semibold text-gray-500">QR</p>
            ...
            <p className="font-semibold mt-3 text-gray-500">Pay with Google pay</p>
            <Link to={pay1}>
          ...
          <div className="text-center mb-4">
            <p className="font-semibold text-gray-500">Copy Account To Pay</p>
            <div className="flex items-center justify-center mt-2">
              <input type="text" value={upi} readOnly className="bg-color-l gray-50 focus:outline-none rounded px-3 py-1 w-full max-w-xs" />
              <button className="ml-2 bgs-green px-3 py-1 rounded" onClick={copyToClipCode}>Copy</button>
            </div>
          </div>

          <div className="flex items-center justify-center mt-4 mx-2">
            <input type="text" placeholder="input 12-digit here" value={utr} onChange={(e) => setUtr(e.target.value)} className="bg-color-l gray-50 focus:outline-none rounded px-3 py-1 w-full max-w-xs" />
          ...
          <div className="text-center text-xs text-gray-500">
            <p>Contact us</p>
            <p>1. please, contact us if you have any payment issue: bdgclub@gmail.com</p>
