        ...
        <>
            <div className='nav-bg p-1 py-3 sticky top-0 z-10'>
                <div className="container-section flex  items-center">
                    <button className='absolute'><Link onClick={handle}>  <IoIosArrowBack className='text-xl text-black' /></Link></button>
            ...
            <div className="container-section mt-5">
                ...
                <div className='nav-bg rounded-md mt-3 pb-7 px-3 pt-1' key={i}>
                    <div className=' mt-3 flex justify-between items-center rounded-sm  text-sm pb-2 border-b border-[--grey-200]'>
                        <span className='fs-sm font-medium bg-red-500  p-2 px-4 rounded-md '>Deposit</span>
                        <span className={`fs-sm font-medium  ${item.status===0?"color-yellow-200":item.status===1?"color-green":"color-red-200"}`}>{item.status===0?"Pending":item.status===1?"Completed":"Failed"}</span>
                    </div>
                    <div className='mt-3 flex justify-between items-center gray-100 text-sm'>
                        <span className='text-sm font-medium'>Balance</span>
                        <span className='text-sm font-medium color-yellow-200'>{item.type==="UPI"?`₹${Number(item.money).toFixed(2)}`:`$${Number((item.money)/93).toFixed(2)}`} </span>
                    </div>
