  ...
  return (
    <>
      ...
      <div className="container-section mt-3">
        <div className='nav-bg rounded-xl p-2 pb-0 text-black'>
          ...
          <div className='bg-gray-100 mt-3  w-[80%]  px-3 py-1 rounded-md mb-2 gray-100 fs-sm '>
            <span>Upgrade VIP level to increase rebet rate</span>
          </div>


          <div className='flex item-center justify-between mb-3'>
            <div className='bg-gray-100  w-[49%] p-2 rounded-md'>
              <p className='text-[12px] gray-100'>Today rebate</p>
              <p className='text-base color-yellow-200 font-medium'>{Number(todayCommission).toFixed(2)}</p>
            </div>
          ...
          <div className=' pb-4 pt-2'>
            <button className={` py-2 rounded-3xl text-base w-full  heading-h3  font-bold  ${userInfo?.rebate>0?"blue-linear color-orange":"bg-gray-200 gray-50"}`} disabled={userInfo?.rebate<1?true:false} onClick={handleSubmit}>One-Click Rebate</button>
          </div>
       
        </div>
        <div className='flex items-center mt-2  border-l-4 border-blue-500 text-sm'>  <h1 className='heading-h1 ml-1 text-black'>Rebate history</h1></div>
        ...
        <button className='border py-1 mt-3 blue-color-300  rounded-3xl text-base w-full border-[var(--main-color)] heading-h3 nav-bg'>All history</button>
      </div>
      {/* alerts */}
